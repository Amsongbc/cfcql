...
class BayesParallelRunner:
    ...
    def save_data(self,data):
        ...
        for agent_idx in range(self.args.n_agents):
            agent_data = first_batch_data[agent_idx].item()  # 获取第 agent_idx 个智能体的数据
            ...
            if save_probs:
                ...
                if self.args.use_bayes:
                    ...
                    for i in range(len(self.macs)):
                        ...
                        _, qvals = self.macs[i].get_Q(self.batch, t_ep=self.t, t_env=self.t_env, bs=envs_not_terminated, test_mode=test_mode,get_Q=True)
                        q_outs.append(qvals)
                    mac_q_sums = []
                    #sum the agents' Q in same mac
                    for q_values in q_outs:
                        mac_q_sum = q_values.sum()
                        mac_q_sums.append(mac_q_sum)

                    mac_probs = th.nn.functional.softmax(th.tensor(mac_q_sums), dim=0)
                    selected_mac_idx = th.multinomial(mac_probs, num_samples=1).item()

                    actions,final_Q=self.macs[selected_mac_idx].get_Q(self.batch, t_ep=self.t, t_env=self.t_env, bs=envs_not_terminated, test_mode=test_mode,get_Q=True)
                    #save Q values in text
                    self.save_data(final_Q)
                else:
