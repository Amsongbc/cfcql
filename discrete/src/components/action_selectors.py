...
class Bayes_EpsilonGreedyActionSelector():
    ...
    def select_action(self, agent_inputs, avail_actions, t_env, test_mode=False):
        ...
        picked_actions = pick_random * random_actions + (1 - pick_random) * masked_q_values.max(dim=2)[1]
        picked_q_values = agent_inputs.gather(2, picked_actions.unsqueeze(-1)).squeeze(-1)
        return picked_actions,picked_q_values

    def select_action_softmax(self, agent_inputs, avail_actions, t_env, test_mode=False):

        # mask actions that are excluded from selection
        masked_q_values = agent_inputs.clone()
        masked_q_values[avail_actions == 0] = -float("inf")  # should never be selected!

        # Apply softmax to the masked Q-values to get action probabilities
        action_probs = th.nn.functional.softmax(masked_q_values, dim=-1)

        # Use Categorical distribution to sample actions from softmax probabilities
        picked_actions = Categorical(action_probs).sample().long()

        return picked_actions


REGISTRY["bayes_epsilon_greedy"] = Bayes_EpsilonGreedyActionSelector


class BayesMultinomialActionSelector():
    ...
    def select_action(self, agent_inputs,agent_q, avail_actions, t_env, test_mode=False):
        masked_policies = agent_inputs.clone()
        masked_policies[avail_actions == 0] = 0
        masked_policies = masked_policies / (masked_policies.sum(-1, keepdim=True) + 1e-8)
        ...
        # 使用 gather 提取 picked_actions 对应的 agent_inputs (即 Q 值或概率)
        picked_q_values = agent_q.gather(2, picked_actions.unsqueeze(-1)).squeeze(-1)

        if self.save_probs:
            return picked_actions, picked_q_values, masked_policies
        else:
            return picked_actions, picked_q_values


REGISTRY["bayes_multinomial"] = BayesMultinomialActionSelector