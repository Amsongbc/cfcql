...
class CQLearner:
    def __init__(self, mac, scheme, logger, args):
        ...
        if getattr(self.args, 'moderate_lambda', False):
            ...
            new_args.mask_before_softmax=True
            new_args.agent = 'n_rnn'
            self.mini_epochs= 1
            new_args.agent_output_type = 'pi_logits'
            new_args.action_selector="multinomial"
            if new_args.use_bayes:
                new_args.action_selector="bayes_multinomial"
            new_args.epsilon_start= .0
            new_args.epsilon_finish= .0
            new_args.epsilon_anneal_time= 500000
            self.behaviour_mac = mac_REGISTRY['basic_mac'](scheme, None, new_args)
            self.behaviour_params = list(self.behaviour_mac.parameters())
            self.behaviour_optimiser = Adam(params=self.behaviour_params, lr=new_args.lr)
    ...
    def train(self, batch: EpisodeBatch, t_env: int, episode_num: int, per_weight=None,use_logger=True):
        # Get the relevant quantities
        rewards = batch["reward"][:, :-1]
        actions = batch["actions"][:, :-1]
        terminated = batch["terminated"][:, :-1].float()
        ...
        if t_env - self.log_stats_t >= self.args.learner_log_interval and use_logger:
